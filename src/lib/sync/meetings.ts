...
import { resolveCompanyId } from "@/lib/sync/run";
 ...
 * la empresa hacia public.meetings, para el gráfico "Horas de acompañamiento
 * por mes" del Inicio. Reconcilia (borra los que ya no existen).
 */
export async function syncMeetings() {
  ...
  // Reconciliación: elimina de la réplica los de esta empresa que ya no están.
  ...
  return { status: "success" as const, upserted: mapped.length };
}
