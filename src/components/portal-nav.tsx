 ...
 * La primera es la puesta en marcha, que es lo que necesita un cliente recién
 * incorporado; el catálogo de herramientas vive dentro de ella.
 */
    ...
    <>
        ...
        <div className="mt-auto flex flex-col gap-3">
          ...
          <button
            ...
            className={cn(
              ...
              collapsed ? "justify-center px-0 py-2.5" : "gap-2.5 px-3 py-2.5",
              readOnly && "hover:bg-card cursor-not-allowed opacity-50",
            )}
          >
      ...
      <header className="border-border bg-background/80 sticky top-0 z-40 flex items-center gap-2 border-b px-4 py-2.5 backdrop-blur-md min-[900px]:hidden">
        ...
        <nav className="flex flex-1 gap-1 overflow-x-auto [mask-image:linear-gradient(to_right,#000_0,#000_calc(100%-24px),transparent_100%)]">
          ...
          {/* Recursos — mismas entradas que la sidebar (Primeros Pasos y
              Memento Mori), con la pastilla accesible de min-h-11. */}
          {RESOURCE_ITEMS.map((item) => {
            const Icon = item.icon;
            return (
              <Link
                key={item.href}
                href={item.href}
                className={pillCls(isActive(item.href))}
              >
                <Icon width={16} height={16} /> {item.label}
              </Link>
            );
          })}
          {/* Scholē solo existía en la sidebar de escritorio. */}
          <a
            href={SCHOLE_URL}
            target="_blank"
            rel="noopener noreferrer"
